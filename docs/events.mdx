---
title: "Events"
description: "The event taxonomy and payloads"
---
...
`execution_id` and `status`, plus `concurrency_key` and `deadline_at` when the
execution has them.

| Type | When |
|------|------|
| `execution.created` | Client created the execution. |
| `execution.started` | The execution began running and its first dispatch is queued. |
