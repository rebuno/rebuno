---
title: "LLM calls"
description: "Intercepting LLM requests so they replay durably"
---
...
[tool call](/tools). An LLM request is not something the agent issues by hand,
...
- Identity is computed over the canonicalized arguments, so any field that varies
  ...
  [step identity](/tools#step-identity).
...
you can gate models or arguments with [policy](/policy).
...
replayed as the whole assembled output. See [live streaming](/streaming).

## Implement interception

The Rebuno SDKs ship an interceptor you drop into your model client, which makes
this transparent. See [Python SDK: LLM calls](/sdk/python/llm-calls).
...
[`examples/gateway/litellm_proxy.py`](https://github.com/rebuno/rebuno/blob/main/examples/gateway/litellm_proxy.py) for a
LiteLLM gateway example, and the [HTTP API](/api#agent-api) for the exact
