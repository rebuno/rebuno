...
- **`args_hash`** — a hash of the arguments, canonicalized by the kernel.
...
different `args_hash`, a different step id — and the kernel treats it as a new
effect and runs it for real. Wrap that non-determinism in
[`rebuno.step()`](steps.md) so the value is recorded once and replayed.

Step arguments must be JSON-serializable (dicts, lists, strings, numbers, bools,
null) — other types raise `TypeError`.
...
- **Agent → kernel:** the agent's `KernelClient` signs every request body the
  ...
  also send `Rebuno-Dispatch-Id`, which scopes the kernel's occurrence counter.
