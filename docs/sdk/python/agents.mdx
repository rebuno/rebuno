...
It runs under an active [execution context](/sdk/python/internals), so `@tool`,
...
1. Verifies the `Rebuno-Signature` header (see [Signing](/sdk/python/internals#signing)).
...
stored result instead of executing again. See [How it works](/sdk/python/internals) for
what breaks this.
...
See [Errors](/sdk/python/errors) for what each exception means.
