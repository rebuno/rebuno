---
title: "How it works"
description: "Step identity, replay, heartbeats, and signing"
---
...
real. Wrap those in [`rebuno.step()`](/sdk/python/steps) instead.
...
[Dispatch and resume](/sdk/python/agents#dispatch-and-resume).
...
the reason for the guidance in [Tools](/sdk/python/tools#blocking-work).
...
- Agent to kernel: `KernelClient` signs the method, URL path and query, the
  timestamp, dispatch lease, and body using the
  [v1 request signature](/api#request-signatures).

[`Client`](/sdk/python/client) uses Bearer auth (`REBUNO_API_KEY`) instead, since it is a
