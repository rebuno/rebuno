...
that has finished, raises `ConflictError`.
...
re-raise if you do. See [Dispatch and resume](/sdk/python/agents#dispatch-and-resume)
...
- `raise_for_refusal(exc)` re-raises a Rebuno refusal a provider SDK wrapped in
  ...
  [LLM calls](/sdk/python/llm-calls#refused-calls).
