---
title: "Agents"
description: "Define an agent and handle its intents"
---

`Agent` serves an HTTP webhook and runs your handler once per dispatch. It runs
under an active [execution context](/sdk/typescript/internals), so `defineTool`,
...
1. Verifies the `Rebuno-Signature` header (see [Signing](/sdk/typescript/internals#signing)).
...
stored result instead of executing again. See [How it works](/sdk/typescript/internals) for
what breaks this.
...
See [Errors](/sdk/typescript/errors) for what each class means.
