---
title: "Clients"
description: "Create executions, inspect them, and resolve approvals"
---
...
const execution = await client.create(
  ...
  { concurrencyKey: "tenant:acme" },   // optional
);
...
all subclasses of `RebunoError`. See [Errors](/sdk/typescript/errors).
...
- `Execution`: `id`, `agentId`, `input`, `status`, `output`,
  `failureReason`, `concurrencyKey`. `status` is an `ExecutionStatus`, one of `pending`,
