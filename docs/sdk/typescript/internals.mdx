...
real. Wrap those in [`step()`](/sdk/typescript/steps) instead.
...
[Dispatch and resume](/sdk/typescript/agents#dispatch-and-resume).
...
guidance in [Tools](/sdk/typescript/tools#blocking-work).
...
- Agent to kernel: the kernel client signs the method, URL path and query, the
  timestamp, dispatch lease, and body using the
  [v1 request signature](/api#request-signatures).

[`Client`](/sdk/typescript/client) uses Bearer auth (`REBUNO_API_KEY`) instead, since it is a
