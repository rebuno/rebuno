---
title: "Architecture"
description: "The domain model, state machines, and how durability works"
---
...
```mermaid
flowchart LR
    client["Client<br>your backend"] -->|create| kernel["Kernel"]
    kernel -->|"signed webhook"| agent["Agent<br>your handler"]
    agent -->|"submit_step, complete"| kernel
    kernel --> db[("Postgres<br>events + steps")]

    classDef core fill:#0697F3,stroke:none,color:#ffffff
    class kernel core
```
...
An execution can carry an immutable `concurrency_key`. Executions sharing a key
run one at a time; the rest wait in `pending`, with no dispatch or delivery
attempts. Different keys run concurrently.

An execution holds its key while `running` or `blocked`, so approval waits,
retry backoff, and rate-limit pauses all keep it, and deadlines include the time
spent waiting. Reaching a terminal state releases the key and starts the oldest
execution waiting on it.
...
replica holds a lock connection; local waiters hold none and leave the queue when
