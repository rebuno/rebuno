---
title: "CLI"
description: "The rebuno binary and its commands"
---
...
Replace `DB_URL` with a Postgres connection string and `TOKEN` with the
full-access bootstrap token.
...
[Deployment](/deployment) for the rest of the server's flags.
...
| `REBUNO_API_KEY` | none | Scoped API key or bootstrap bearer token. Required against `rebuno server`. |
...
Set `REBUNO_API_KEY` to the bootstrap token or a key with `api_keys:manage`.

```bash
rebuno key create reader --scope executions:read
rebuno key ls
rebuno key rotate <id>
rebuno key revoke <id>
```

Output is JSON. Repeat `--scope` or use commas for multiple
[scopes](/api#scopes). Save issued tokens; they are shown once.
See [API keys](/api#api-keys) for rotation and revocation.
...
| `rebuno exec cancel <id>` | Cancel a pending, running, or blocked execution. |

`exec ls` narrows with `--agent`, `--concurrency-key`, `--status`, and `--limit`.
...
See [Policy](/policy) for what puts a step in front of you.
...
See [Testing a bundle](/policy#testing-a-bundle).
