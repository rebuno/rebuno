---
title: "Agents"
description: "The webhook protocol and the dispatch and replay lifecycle"
---
...
[Provisioning agents](/deployment#provisioning-agents).
...
See the [HTTP API](/api) for the exact request and response shapes, and
[Architecture](/architecture) for how step identity makes replay work.
...
[LLM calls](/llm-calls) explains how that works, and also how to wire it up to your
own gateway.
...
- If something non-deterministic decides which effects fire, record it as a
  ...
  expose this as `step(name, fn)`. See [Local steps](/tools#local-steps).
...
declared. See [Idempotency modes](/tools#idempotency-modes).
