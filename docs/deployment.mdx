---
title: "Deployment"
description: "Running the production kernel, its config, and Docker"
---
...
Client and admin routes accept scoped API keys or the full-access bootstrap
bearer token:
...
[`examples/rebuno.dev.yaml`](https://github.com/rebuno/rebuno/blob/main/examples/rebuno.dev.yaml):
...
agents and load policy at runtime over the [admin API](/api#admin-api).
...
| `REBUNO_RETENTION` | `24h` | Creation-age cutoff for deleting terminal executions. Pending, running, and blocked executions are retained. |
| `REBUNO_LEADER_LOCK_KEY` | `rebuno_scheduler_leader` | Advisory-lock key for leader election. |
| `REBUNO_TYPESAFE_API_KEY` | none | TypeSafe API key for policy rules that decide `judge`. Without it, those rules deny. See [Judge](/policy#judge). |
...
The image is built from [`deploy/Dockerfile`](https://github.com/rebuno/rebuno/blob/main/deploy/Dockerfile) and published
