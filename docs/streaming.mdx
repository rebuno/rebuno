...
description: "Live token deltas while a step is running"
---
...
Each delta carries the dispatch lease headers, like the step mutations. The
kernel accepts a delta only from the current dispatch while the step is
executing: a superseded attempt gets `409 lease_superseded`, and a step that has
finished or was never started gets `409 conflict`. Neither check renews the
lease.
