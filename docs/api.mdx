...
Add an optional `concurrency_key` to keep executions that share it from running
at the same time:

```json
{
  "agent_id": "researcher",
  "concurrency_key": "tenant:acme:report:42",
  "input": { "query": "hello" }
}
```

The execution returns `running` when the key is free and `pending` when another
execution holds it. Keys are global to the kernel, so executions for two
different agents that share a key also run one at a time. See
[Execution concurrency](/architecture#execution-concurrency).
...
`GET /v0/executions?agent_id=&concurrency_key=&status=&cursor=&limit=` · bearer

Newest first. Keyset paging: pass the returned `next_cursor` back as `cursor`.
`concurrency_key` filters by exact key.
...
delivered best-effort. `data` is opaque to the kernel and capped per batch. A
step that is no longer executing returns `409 conflict`. See
[Live streaming](/streaming).
